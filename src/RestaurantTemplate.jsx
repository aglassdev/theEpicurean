...
const RestaurantPage = ({
  ...
  React.useEffect(() => {
    if (pageTitle) {
      ...
      document.title = `${restaurantName} - The Epicurean`;
    }
  }, [pageTitle, restaurantName]);

  // Parse reservation providers if it's a string
  const parsedReservationProviders = React.useMemo(() => {
    if (!reservationProviders) return [];
    
    if (typeof reservationProviders === 'string') {
      try {
        // The JSON might contain JavaScript code, so we need to eval it
        const result = eval(reservationProviders);
    ...
    return Array.isArray(reservationProviders) ? reservationProviders : [];
  }, [reservationProviders]);
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                ...
                <div style={{
                  ...
                  {parsedReservationProviders?.map((provider, index) => (
                    <a
                      key={index}
                      href={provider.url}
                      target="_blank"
